...
Based in Sydney, Australia. aitutormaths.com  ·  larrydu2002@users.noreply.github.com
---
...
Status	Built solo · runs locally · live demo available	Live in production at aitutormaths.com
...
Product-Listing Agent — a genuine agent, built on the Anthropic SDK as a hand-written tool-use loop (no framework). Given a sparse product and a set of tools, the model decides which tool to call, in what order, and when it's done. The control flow belongs to the model, not my code — my loop only executes the tool the model names and hands the result back.
